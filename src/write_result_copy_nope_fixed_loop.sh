...
base_path = Path("../models/nope_baselines/copy_fixed_loop")
all_dirs = [d for d in base_path.iterdir() if d.is_dir()]
if not all_dirs:
    print("Error: No fixed-loop experiment directories found.")
    raise SystemExit(1)

run_dir = sorted(all_dirs, key=lambda p: p.stat().st_mtime)[-1]
csv_path = Path("../eval/nope_copy_fixed_loop") / run_dir.name / "diagnostics_loops40" / "summary_by_length.csv"

if not csv_path.exists():
    print(f"Error: CSV file not found at {csv_path}")
    print("Hint: run bash run_eval_copy_nope_fixed_loop.sh first.")
    raise SystemExit(1)
...
output_file = "nope_copy_fixed_loop.txt"
