...
base_path = Path("../models/rope_baselines/copy_fixed_loop")
...
csv_path = Path("../eval/rope_copy_fixed_loop") / run_dir.name / "diagnostics_loops40" / "summary_by_length.csv"

if not csv_path.exists():
    print(f"Error: CSV file not found at {csv_path}")
    print("Hint: run bash run_eval_copy_rope_fixed_loop.sh first.")
    raise SystemExit(1)
...
output_file = "rope_copy_fixed_loop.txt"
