...
# W&B
USE_WANDB = True
WANDB_PROJECT = "looped-tf-rope-copy-ponder"
    ...
    if cfg.wandb.enabled:
        ...
        if UPLOAD_DIAGNOSTIC_ARTIFACT:
            artifact = wandb.Artifact(
                name=f"{cfg.task}_rope_ponder_diagnostics",
                type="diagnostics",
            )
...
def train(cfg):
    ...
    if cfg.wandb.enabled:
        wandb.init(
            ...
            mode=cfg.wandb.mode,
            name=f"{cfg.task}_rope_ponder_seed{cfg.seed}_{run_id[:8]}",
            dir=out_dir,
    ...
    if cfg.wandb.enabled and UPLOAD_MODEL_ARTIFACT:
        artifact = wandb.Artifact(
            name=f"{cfg.task}_rope_ponder_{run_id[:8]}",
            type="model",
        )
