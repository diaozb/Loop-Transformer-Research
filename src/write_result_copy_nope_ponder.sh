...
base_path = Path("../models/nope_baselines/copy_ponder")
...
csv_path = Path("../eval/nope_copy_ponder") / run_dir.name / "diagnostics_steps20" / "summary_by_length.csv"

if not csv_path.exists():
    print(f"Error: CSV file not found at {csv_path}")
    print("Hint: run bash run_eval_copy_nope_ponder.sh first.")
    raise SystemExit(1)
...
output_file = "nope_copy_ponder.txt"
