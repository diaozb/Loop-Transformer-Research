...
from generate_training_data import generate_prompt_matrix_mod_add
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate mod_add model across lengths and loop counts.")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        help="Comma-separated mod_add lengths (number of terms).",
    )
    parser.add_argument(
        "--loop_counts",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=11,
        help="Modulus for mod_add task.",
    )
    parser.add_argument(
        "--run_name",
        type=str,
        default=None,
        help="Optional run name for output directory (subfolder under eval/mod_add/<checkpoint_name>/).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Optional full output directory. If set, overrides the default eval/mod_add/<checkpoint_name>/<run_name>.",
    )
    ...
    if args.output_dir:
        ...
        output_dir = os.path.join(repo_root, "eval", "mod_add", checkpoint_name, run_name)
    ...
    for length in lengths:
        ...
        while num_done < args.num_samples:
            ...
            if linear_embedding:
                xs = torch.tensor(convert_to_one_hot(xs.numpy(), n_dims=args.modulus + 2), dtype=torch.float32, device=device)
            else:
                xs = xs.to(device)

            with torch.no_grad():
                hidden_list, logits_list = _looped_forward_collect(model, xs, horizon)
                batch_metrics = _batch_metrics(hidden_list, logits_list, ys, mask, loop_counts, pad_token=args.modulus)
    ...
    # Save JSON
    json_path = os.path.join(output_dir, "mod_add_eval_results.json")
    ...
    # Save CSV
    csv_path = os.path.join(output_dir, "mod_add_eval_results.csv")
