...
def _batch_metrics(
    ...
    for idx, lc in enumerate(loop_counts):
        ...
        # L2 norm of hidden delta vs loop t-cycle on masked positions.
        for cycle in cycles:
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate parity model across lengths and loop counts.")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        help="Comma-separated parity lengths (number of digits).",
    )
    parser.add_argument(
        "--loop_counts",
        type=str,
    ...
    parser.add_argument(
        ...
        type=int,
        default=1,
        help="Max cycle for H_t - H_{t-cycle} norm on masked positions (plots for 1..cycle).",
    )
    parser.add_argument(
        "--num_samples",
        type=int,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional run name for output directory (subfolder under eval/parity/<checkpoint_name>/).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Optional full output directory. If set, overrides the default eval/parity/<checkpoint_name>/<run_name>.",
    )
    ...
    if args.output_dir:
        ...
        output_dir = os.path.join(repo_root, "eval", "parity", checkpoint_name, run_name)
    ...
    # Save JSON
    json_path = os.path.join(output_dir, "parity_eval_results.json")
    ...
    # Save CSV
    csv_path = os.path.join(output_dir, "parity_eval_results.csv")
