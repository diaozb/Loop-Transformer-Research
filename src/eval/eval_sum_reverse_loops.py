...
from generate_training_data import generate_prompt_matrix_sum_reverse
...
def _batch_metrics(
    ...
    for idx, lc in enumerate(loop_counts):
        ...
        answer_correct = torch.ones(batch_size, dtype=torch.bool, device=ys.device)
        for i in range(batch_size):
            ans_idx = answer_mask[i]
            if ans_idx.any():
                answer_correct[i] = (preds[i, ans_idx] == ys[i, ans_idx]).all()
        answer_acc = answer_correct.float().mean().item()
...
def main():
    parser = argparse.ArgumentParser(description="Evaluate sum_reverse model across lengths and loop counts.")
    parser.add_argument(
        "--checkpoint",
        type=str,
        required=True,
    ...
    parser.add_argument(
        ...
        help="Comma-separated sum_reverse lengths (number of digits).",
    )
    parser.add_argument(
        "--loop_counts",
        type=str,
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        help="Optional run name for output directory (subfolder under eval/sum_reverse/<checkpoint_name>/).",
    )
    parser.add_argument(
        "--output_dir",
        type=str,
        default=None,
        help="Optional full output directory. If set, overrides the default eval/sum_reverse/<checkpoint_name>/<run_name>.",
    )
    ...
    if args.output_dir:
        ...
        output_dir = os.path.join(repo_root, "eval", "sum_reverse", checkpoint_name, run_name)
    ...
    for length in lengths:
        ...
        while num_done < args.num_samples:
            ...
            xs, batch_num, ys, mask = generate_prompt_matrix_sum_reverse(
    ...
    # Save JSON
    json_path = os.path.join(output_dir, "sum_reverse_eval_results.json")
    ...
    # Save CSV
    csv_path = os.path.join(output_dir, "sum_reverse_eval_results.csv")
