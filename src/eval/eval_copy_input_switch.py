...
CKPT_PATH = "/data/yizhou/looped-tf-length-generalization/models/copy/8fe7447f-9d64-4de4-bf04-29c50e860cd6"
LENGTH_L = 30
NUM_SAMPLES = 2000
BATCH_SIZE = 1024
DEVICE = "cuda"
...
NOISE_SIGMA = 0.1
ADAPTIVE_NOISE_SIGMA = True
RUN_STEPS_MODE = "extra_after_switch"  # choices: "total_l", "extra_after_switch"
# ---------------------------------------
...
def _make_b_from_a_copy(xs_a: torch.Tensor, ys_a: torch.Tensor, batch_num: torch.Tensor):
    xs_b = xs_a.clone()
    ys_b = ys_a.clone()
    b = xs_a.shape[0]
...
def _answer_mask(batch_num: torch.Tensor, seq_len: int, device: torch.device) -> torch.Tensor:
    ...
    start = batch_num.to(device=device)
    end = (batch_num * 2).to(device=device)
...
def _adaptive_sigma_from_reference(reference: torch.Tensor, eps: float = 1e-12) -> torch.Tensor:
    # Match Gaussian RMS scale to reference injection RMS scale per sample.
    # reference: [batch, seq, dim] -> sigma: [batch, 1, 1]
    rms = reference.float().pow(2).mean(dim=(1, 2), keepdim=True).sqrt()
    return torch.clamp(rms, min=eps)


def _run_switch_experiment(model, device: torch.device, use_wpe: bool):
    torch.manual_seed(SEED)
    if device.type == "cuda":
        torch.cuda.manual_seed_all(SEED)

    num_done = 0
    if RUN_STEPS_MODE == "total_l":
        switch_steps = list(range(2, LENGTH_L + 1))
    elif RUN_STEPS_MODE == "extra_after_switch":
        switch_steps = list(range(2, LENGTH_L + 2))
    else:
        raise ValueError("RUN_STEPS_MODE must be either 'total_l' or 'extra_after_switch'.")
    ...
    while num_done < NUM_SAMPLES:
        ...
        xs_A, batch_num_A, ys_A, _ = generate_prompt_matrix_copy(
            b, max_len=LENGTH_L + 1, min_num_digits=LENGTH_L, max_num_digits=LENGTH_L + 1
        )
        xs_B, ys_B = _make_b_from_a_copy(xs_A, ys_A, batch_num_A)
        ...
        with torch.no_grad():
            ...
            for switch_step in switch_steps:
                ...
                if RUN_STEPS_MODE == "total_l":
                    ...
                    total_steps = switch_step + LENGTH_L - 1
                for step in range(1, total_steps + 1):
                    if step < switch_step:
                        ...
                        if RANDOM_INJECTION:
                            sigma = NOISE_SIGMA
                            if ADAPTIVE_NOISE_SIGMA:
                                sigma = _adaptive_sigma_from_reference(zs_B)
...
def main():
    ...
    ce_avg, acc_avg = _run_switch_experiment(model, device, use_wpe=use_wpe)
    ...
    if RUN_STEPS_MODE == "total_l":
        ...
        iter_steps = range(2, LENGTH_L + 1)
    else:
        print("Final-step comparison for each switch step k (switch at step k, then loop L more steps):")
        iter_steps = range(2, LENGTH_L + 2)
    for k in iter_steps:
        ce_a = ce_avg[k]["A"]
        ce_b = ce_avg[k]["B"]
        acc_a = acc_avg[k]["A"]
