...
base_path = Path("../models/ape_once_baselines/copy_ponder")
...
csv_path = Path("../eval/ape_once_copy_ponder") / run_dir.name / "diagnostics_steps20" / "summary_by_length.csv"

if not csv_path.exists():
    print(f"Error: CSV file not found at {csv_path}")
    print("Hint: run bash run_eval_copy_ape_once_ponder.sh first.")
    raise SystemExit(1)
...
output_file = "ape_once_copy_ponder.txt"
