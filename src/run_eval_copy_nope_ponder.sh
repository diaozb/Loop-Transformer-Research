...
RUN_DIR=$(ls -td ../models/nope_baselines/copy_ponder/* | head -1)
RUN_ID=$(basename "$RUN_DIR")
OUT_DIR="../eval/nope_copy_ponder/${RUN_ID}/diagnostics_steps20"
...
python eval_ponder_diagnostics.py \
  ...
  --out-dir "$OUT_DIR" \
  --wandb \
  --wandb-project looped-tf-nope-copy-ponder \
  --wandb-name "eval_nope_copy_ponder_${RUN_ID}_steps20"
